...
export default function TransporteursPage() {
  ...
  const type = PRESTA_TYPES.transporteurs;
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h5" sx={{ fontWeight: 700 }}>Transporteurs</Typography>
        <Typography variant="body2" color="text.secondary">
          {filtered.length} transporteur{filtered.length > 1 ? 's' : ''} — prestataires logistique Hanyjay
        </Typography>
      </Box>
