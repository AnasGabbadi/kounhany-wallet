...
export default function GaragesPage() {
  ...
  const type = PRESTA_TYPES.garages;
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h5" sx={{ fontWeight: 700 }}>Garages partenaires</Typography>
        <Typography variant="body2" color="text.secondary">
          {filtered.length} garage{filtered.length > 1 ? 's' : ''} — prestataires de service
        </Typography>
      </Box>
        ...
        <CardContent sx={{ p: 3, flex: 1, display: 'flex', flexDirection: 'column', '&:last-child': { pb: 2 } }}>
          {isLoading ? (
            ...
            <PrestatairesTable
              ...
              onWallet={(id) => goToWallet(id, 'garages')}
            />
          )}
        </CardContent>
      </Card>
