...
export default function B2CPage() {
  ...
  const b2cClients = clients
    .filter(c => !c.client_id?.startsWith('company_') && c.client_type === 'B2C')
    .filter(c =>
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h5" sx={{ fontWeight: 700 }}>Comptes particuliers</Typography>
        <Typography variant="body2" color="text.secondary">
          {b2cClients.length} client{b2cClients.length > 1 ? 's' : ''} B2C — wallets individuels
        </Typography>
      </Box>
