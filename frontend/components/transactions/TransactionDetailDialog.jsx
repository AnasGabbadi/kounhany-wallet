...
export default function TransactionDetailDialog({ tx, onClose }) {
  ...
  return (
    <Dialog open={!!tx} onClose={onClose} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
      <DialogTitle sx={{ p: 3, pb: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6" sx={{ fontWeight: 700 }}>Détail de la transaction</Typography>
      ...
      <DialogContent sx={{ p: 3, pt: 0 }}>
        <Box sx={{ p: 2, borderRadius: 2, bgcolor: 'rgba(250,195,69,0.06)', border: '1px solid rgba(250,195,69,0.2)', mb: 2.5, textAlign: 'center' }}>
          ...
          <Typography variant="h4" sx={{ fontWeight: 700, color: '#212529' }}>{fmt(tx.amount)} MAD</Typography>
        ...
        {rows.map((row) => (
          <Box key={row.label} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1, borderBottom: '1px solid rgba(0,0,0,0.05)', '&:last-child': { borderBottom: 'none' } }}>
            <Typography variant="body2" color="text.secondary" sx={{ minWidth: 120 }}>{row.label}</Typography>
            {typeof row.value === 'string' ? (
              <Typography variant="body2" sx={{ fontWeight: row.bold ? 700 : 500, fontFamily: row.mono ? 'monospace' : 'inherit', fontSize: row.mono ? '0.75rem' : 'inherit', textAlign: 'right', maxWidth: 280, wordBreak: 'break-all' }}>
                {row.value}
              </Typography>
            ) : row.value}
          </Box>
      ...
      <DialogActions sx={{ p: 3, pt: 1, gap: 1 }}>
        ...
        <Button
          variant="outlined"
          onClick={() => { router.push(getWalletUrl(tx.client_id)); onClose(); }}
          sx={{ borderColor: '#FAC345', color: '#E0A820', fontWeight: 600 }}
        >
          {tx.client_id?.startsWith('prestataire_') ? 'Voir le wallet prestataire' : 'Voir le wallet client'}
        </Button>
      </DialogActions>
    </Dialog>
  );
